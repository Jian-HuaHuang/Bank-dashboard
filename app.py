...
# 可建模跨售的目標商品（存款近100%滲透、無跨售空間故不列入）
...
# ================= 頁面 3：跨售商機 =================
    ...
    flag = TARGET_FLAG[t_name]
    pcol = TARGET_COL[t_name]
    ...
    with_t = C[C[flag] == 1].AnnualContribution_k
    without_t = C[C[flag] == 0].AnnualContribution_k
...
# ================= 頁面 4：模型洞察 =================
    ...
    if METRICS is not None:
        ...
        mt = METRICS.rename(columns={"test_auc": "測試AUC", "cv_auc": "CV-AUC"})
    ...
    with c1:
        ...
        if ROC is not None:
            fig = go.Figure()
            palette = {"信用卡": RED, "放款": GOLD, "財富管理": MAROON, "保險": SOFT}
            for nm in TARGET_NAMES:
                sub = ROC[ROC["商品"] == nm].sort_values("fpr")
                auc = ""
